  ...
  try {
    ...
    const [successRes, failRes, failCountRes] = await Promise.all([
      supabaseClient
        ...
        .select("upi_id, timestamp, memo")
        .eq("type", "SUCCESS")
        .order("created_at", { ascending: false }),
      supabaseClient
        .from(SUPABASE_CONFIG.tableName || "upi_logs")
        .select("upi_id, timestamp")
        .eq("type", "FAIL")
        .order("created_at", { ascending: false })
        .limit(1000),
      supabaseClient
      ...
      failRes.data.forEach(row => {
        ...
        if (!state.failedSet.has(id)) {
          ...
          state.failedList.unshift({
            id: id,
            timestamp: row.timestamp || getFormattedNow()
          });
          updated = true;
        }
      });
    ...
    const totalFailed = failCountRes.count !== null && failCountRes.count !== undefined 
      ? failCountRes.count 
...
function initEmergencyNotice() {
  ...
  const btnDevReopen = document.getElementById("btnDevReopenEmergency");
  ...
  if (btnDevReopen) {
    btnDevReopen.addEventListener("click", () => {
      openEmergencyModal();
    });
  }
